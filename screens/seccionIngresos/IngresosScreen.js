...
import { getTotalPriceJSON } from '../../asyncStoreControl/aStorageController';
...
const IngresosScreen = ({ navigation, route }) => {
    ...
    const loadGastos = async () => {
        const data = await getTotalPriceJSON('@gastosKey');
        setGastos(data);
    }

    const loadIngresos = async () => {
        const data = await getTotalPriceJSON('@ingresosKey');
        setIngresos(data);
    }

    const loadSaldo = async () => {
        const ingre = await getTotalPriceJSON('@ingresosKey');
        const gast = await getTotalPriceJSON('@gastosKey');
