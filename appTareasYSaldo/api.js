...
const APIDEUDAS = "http://192.168.1.13:3000/deudas";
...
//seccion deudas
export const getDeudas = async (id) => {
    const res = await fetch(`${APIDEUDAS}/user/${id}`)

    return await res.json();
}

export const getDeuda = async (id) => {
    const res = await fetch(`${APIDEUDAS}/${id}`)
    return await res.json()
}


export const saveDeuda = async (newdeuda) => {
    const res = await fetch(APIDEUDAS, {
        method: 'POST',
        headers: { Accept: "application/json", "Content-Type": 'application/json' },
        body: JSON.stringify(newdeuda)
    });
    console.log(JSON.stringify(newdeuda))
    return await res.json();
};
...
export const updateDeuda = async (id, newdeuda) => {
    const res = await fetch(`${APIDEUDAS}/${id}`, {
        method: 'PUT',
        headers: { Accept: "application/json", "content-Type": "application/json" },
        body: JSON.stringify(newdeuda),
